...
const vertexShader = /* glsl */ `
  ...
  void main() {
    vNormal = normal;
    float n = snoise(position * uFreq + uTime * 0.4);
    vDisp = n;
...
const fragmentShader = /* glsl */ `
  ...
  void main() {
    float mixAmt = smoothstep(-0.4, 0.6, vDisp);
...
export default function DistortedSphere({ pointer }: Props) {
  ...
  const uniforms = useMemo(
    () => ({
      ...
      uColorB: { value: new THREE.Color('#FF2EC4') },
    }),
    []
  );
